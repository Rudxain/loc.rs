...
fn f_loc(mut f: File, buf: &mut Vec<u8>, err: &mut io::Stderr) -> io::Result<usize> {
...
fn recursive_loc(p: &Path, buf: &mut Vec<u8>, err: &mut io::Stderr) -> io::Result<usize> {
	let mut total = 0;
...
fn main() -> io::Result<()> {
	...
	if argv.is_empty() {
		...
		/*
		We could do bin-search to insert elements
		while keeping the slice sorted,
		but the time-complexity suggests that's worse:
		- multi-push then single-sort: O(n + n*log(n)) = O(n log n)
		- multi-binary-insert: O(((n*(n-1)) / 2) = O(n^2)
		*/
